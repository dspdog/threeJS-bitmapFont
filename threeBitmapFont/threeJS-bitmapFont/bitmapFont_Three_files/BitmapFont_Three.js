...
var BitmapFont = function(){
    ...
    this.pointGeomsPerLetterObj = {};
    this.pointTotalsPerLetterObj = {};

    this.validLettersStr.split("").map(function(char){
        var sop = scope.getPtArr_byUnicodeLetter(char);
        var geom = setOfPts2Geom(sop);
        scope.pointGeomsPerLetterObj[char]=geom;
        //return geom;
    });

    this.validLettersStr.split("").map(function(char){
        var sop = scope.getPtArr_byUnicodeLetter(char);
        scope.pointTotalsPerLetterObj[char]=sop.length;
    });

    //this.getValidLettersStr();
};

BitmapFont.prototype.ptsInString = function(str){
    var total=0;
...
BitmapFont.prototype.threeGeomForLetter = function(letter){
    //if(this.validLettersStr.indexOf(letter)==-1)return null;
    //return this.pointGeomsPerLetter[this.validLettersStr.indexOf(letter)];
    return this.pointGeomsPerLetterObj[letter];
}
...
BitmapFont.prototype.threeGeomForString = function(str, _lineLen){
    var scope = this;
    var xPerLetter = 10.0;
    var yPerLetter = 10.0;
...
BitmapFont.prototype.threeGeomForString_direct = function(str, _lineLen){
    var scope = this;
    var xPerLetter = 8.0;
    var yPerLetter = 8.0;
    var lineLen = _lineLen || 32.0;
    ...
    for(var i=0; i<str.length; i++){
        var xs = (i%lineLen)*xPerLetter;
        var ys = Math.floor(i/lineLen)*-yPerLetter;
        var index = this.getIndexForLetter(str.charAt(i));
        index = clamp(index,0, scope.font8x8_allChars.length-1);
        ...
        while(x--){
            y=8;
            while(y--){
                if(bitMap[x] & 1 << y)geometry.vertices.push( new THREE.Vector3(y+xs, -x+ys, 0) );
            }
        }
    }
    return geometry;
...
BitmapFont.prototype.threeGeomForString_direct_buffer = function(str, _lineLen){ //TODO -- fast but leaves 1 extra pixel in upper left, why? + weird error about bounding sphere
    var scope = this;
    var xPerLetter = 8.0;
    var yPerLetter = 8.0;
    var lineLen = _lineLen || 32.0;
    var geometry = new THREE.BufferGeometry();
    var vertices = new Float32Array(this.ptsInString(str)*3.0);
    var x,y,xs,ys,index,bitMap;
    var vi=0;
    var sfl = scope.font8x8_allChars.length-1;

    //var i =

    for(var i=0; i<str.length; i++){
        xs = (i%lineLen)*xPerLetter;
        ys = Math.floor(i/lineLen)*-yPerLetter;
        index = this.getIndexForLetter(str.charAt(i));
        index = clamp(index,0, sfl);
        bitMap = this.font8x8_allChars[index];
        x=8;
        while(x--){
            y=8;
            while(y--){
...
BitmapFont.prototype.getPtMeshForString = function(str, material, lineLen){
    return new THREE.Points( bf.threeGeomForString_direct_buffer(str, lineLen), material || new THREE.PointsMaterial( { size: 2.0, color: 0xff00ff } ) );
}

BitmapFont.prototype.getPtMeshForLetter = function(letter, material){
    return new THREE.Points( bf.threeGeomForLetter(letter), material || new THREE.PointsMaterial( { size: 2.0, color: 0xff00ff } ));
}
...
BitmapFont.prototype.getPtArr_byString = function(str){
    var scope = this;
    return str.split("").map(function(letter){
